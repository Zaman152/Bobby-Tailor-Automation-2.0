...
### Accuracy strategy (LOCKED)
- Target: highest achievable plans-only accuracy on image analysis and calculations; 100% completeness (every expected item present or explicitly flagged `needs_review` — zero silent misses).
- Vector-first measurement: when the PDF has a vector/text layer (most CAD exports do — ~68k geometry items/sheet on fixtures), lengths/areas must come from geometry + detected scale, not from vision estimates. Vision's role shifts to semantic labeling (what is this region/symbol), classification, and reading text/schedules.
- Auto scale detection is required (user directive: "auto detect the scale mentioned"). Must handle multi-scale sheets (main plan viewport vs detail callouts) — known failure: frequency-based dominant scale picks detail scales; dimension self-calibration landed 1"=24' vs true ~1"=20'.
- Ensemble/self-consistency: known failure is run-to-run variance (397,500 SF vs 1,131,112 SF for the same item on identical runs). Multiple extraction runs + voting, and agreement-based confidence, are required for vision-derived numbers.
